...
public:
	...
	void copy_from_wl_shm_data(void const* input_data, Vec2i input_size, uint32_t format,
	...
	void copy_into_rgb_buffer(ColorRGB* output_data, Vec2i output_size, Vec2i lower_left, ColorRGB on_color,
							  ColorRGB off_color);
	void copy_into_packed_bit_buffer(void* outout_data, Vec2i output_size, Vec2i lower_left);
