...
void logMessage(string source, MessageType type, string message)
{
	...
	switch (type)
	{
	case MESSAGE_DEBUG: typeStr = "debug"; break;
	case MESSAGE_WARNING: typeStr = "WARNING"; break;
	case MESSAGE_ASSERTION_FAILED: typeStr = "FATAL ASSERTION"; break;
	case MESSAGE_FATAL_ERROR: typeStr = "FATAL ERROR"; break;
	default: typeStr = "UNKNOWN MESSAGE TYPE";
	}
	string msg = "[" + source + "]: " + message;
	vector<string> lines;
	int messageWidth = std::max(termWidth - (int)typeStr.size(), 12);
	// string indent = [&]() -> string { string out = ""; for (int i = 0; i < (int)typeStr.size() - 1; i++) { out += "
	// "; } return out; } ();
