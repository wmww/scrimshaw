...
const struct wl_surface_interface WlSurface::Impl::surfaceInterface = {
	...
	.set_input_region = +[](wl_client* client, wl_resource* resource,
							wl_resource* region) { debug("wl_surface.set_input_region not implemented"); },
	.commit =
		+[](wl_client* client, wl_resource* resource) {
			...
			if (buffer != nullptr && impl->isDamaged)
			{
				...
				// bool idkWhatThisVarMeans = Impl::eglQueryWaylandBufferWL(display, buffer, EGL_TEXTURE_FORMAT,
				// &texture_format);
