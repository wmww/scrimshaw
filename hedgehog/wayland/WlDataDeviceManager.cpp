...
{
	// start_drag
	+ [](struct wl_client* client, struct wl_resource* resource, struct wl_resource* source, struct wl_resource* origin,
		 struct wl_resource* icon,
		 uint32_t serial) { warning("wl_data_device_interface.start_drag not yet implemented"); },
		// set_selection
...
WlDataDeviceManager::WlDataDeviceManager(wl_client* client, uint32_t id, uint version)
{
	...
	impl->dataDeviceManager.setup(
		impl, client, id, &wl_data_device_manager_interface, version, &Impl::dataDeviceManagerInterface);
}
