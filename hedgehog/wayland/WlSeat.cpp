...
const struct wl_pointer_interface WlSeat::Impl::pointerInterface = {
	...
	.set_cursor = +[](wl_client* client, wl_resource* resource, uint32_t serial, wl_resource* _surface,
					  int32_t hotspot_x, int32_t hotspot_y) { debug("wl_pointer.set_cursor ignored"); },
...
const struct wl_seat_interface WlSeat::Impl::seatInterface = {
	...
	.get_pointer =
		+[](wl_client* client, wl_resource* resource, uint32_t id) {
			...
			impl->pointer.setup(
				impl, client, id, &wl_pointer_interface, wl_resource_get_version(resource), &pointerInterface);
		},
	.get_keyboard =
		+[](wl_client* client, wl_resource* resource, uint32_t id) {
			...
			impl->keyboard.setup(
				impl, client, id, &wl_keyboard_interface, wl_resource_get_version(resource), &keyboardInterface);
	...
	.get_touch =
		+[](wl_client* client, wl_resource* resource, uint32_t id) { warning("wl_seat.get_touch not implemented"); },
	.release =
		+[](wl_client* client, wl_resource* resource) {
...
void WlSeat::pointerMotion(V2d position, Resource surface)
{
	...
	else
	{
		...
		{
			wl_pointer_send_motion(pointer.getRaw(), timeSinceStartMili(), fixedPos.x, fixedPos.y);
		}
	}
...
void WlSeat::pointerClick(uint button, bool down, Resource surface)
{
	...
	{
		wl_pointer_send_button(pointer.getRaw(),
