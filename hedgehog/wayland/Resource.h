...
#define IMPL_ELSE(action)                      \
	shared_ptr<Impl> impl = this->impl.lock(); \
	ASSERT_ELSE(impl, action);
#define IMPL_FROM(resource)                                 \
	shared_ptr<Impl> impl = Resource(resource).get<Impl>(); \
	if (!impl)                                              \
	{                                                       \
		warning(FUNC + " called with invalid resource");    \
		return;                                             \
	}
...
public:
	class Data
	{}; // the only purpose of this is to inherit from

	Resource() {}
	// Resource(shared_ptr<Data> dataIn, wl_client * client, uint32_t id, const wl_interface * interface, int version,
	// const void * implStruct);
	void setup(shared_ptr<Data> dataIn, wl_client* client, uint32_t id, const wl_interface* interface, int version,
			   const void* implStruct);
