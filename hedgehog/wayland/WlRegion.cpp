...
{
	bool checkPoint(V2d pt) { return false; }
};
...
struct RectArea : Area
{
	...
	bool checkPoint(V2d pt) { return pt.x >= low.x && pt.y >= low.y && pt.x <= high.x && pt.y <= high.y; }
};
...
struct UnionArea : Area
{
	...
	bool checkPoint(V2d pt) { return a->checkPoint(pt) || b->checkPoint(pt); }
};
...
struct InverseArea : Area
{
	...
	InverseArea(unique_ptr<Area> a) { this->a = move(a); }

	bool checkPoint(V2d pt) { return !a->checkPoint(pt); }
};
...
const struct wl_region_interface WlRegion::Impl::wlRegionInterface = {
	...
	+[](struct wl_client* client, struct wl_resource* resource, int32_t x, int32_t y, int32_t width, int32_t height) {
		...
		// assert(impl);
		IMPL_FROM(resource);
		if (x == INT32_MIN && y == INT32_MIN && width == INT32_MAX && height == INT32_MAX)
		{
