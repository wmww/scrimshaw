...
struct XdgShellV6Surface::Impl : Resource::Data, WindowInterface
{
	...
	void setSize(V2i size) { warning(FUNC + " not yet implemented"); }

	weak_ptr<InputInterface> getInputInterface() { return waylandSurface.getInputInterface(); }
...
const struct zxdg_surface_v6_interface XdgShellV6Surface::Impl::xdgSurfaceV6Interface = {
	...
	+[](struct wl_client* client, struct wl_resource* resource, uint32_t id) {
		...
		*((zxdg_toplevel_v6_state*)wl_array_add(&states, sizeof(zxdg_toplevel_v6_state))) =
		ZXDG_TOPLEVEL_V6_STATE_ACTIVATED; zxdg_toplevel_v6_send_configure(impl->xdgToplevelResource.getRaw(), 0, 0,
		&states); wl_array_release(&states);
		*/
	...
	// get_popup
	+[](struct wl_client* client, struct wl_resource* resource, uint32_t id, struct wl_resource* parent,
	...
	+[](struct wl_client* client, struct wl_resource* resource, uint32_t serial) {
		...
		// this is useful if we need to know when a client has responded to a configure event, but that is not needed
		// now
	}};
...
const struct zxdg_toplevel_v6_interface XdgShellV6Surface::Impl::xdgToplevelV6Interface = {
	...
	// show_window_menu
	+[](struct wl_client* client, struct wl_resource* resource, struct wl_resource* seat, uint32_t serial, int32_t x,
		int32_t y) {
		debug("zxdg_toplevel_v6_interface::show_window_menu called, ignoring");
		// this dumb ass message is for the compositor to pop up a window menu when client side decorations are right
		// clicked on fuck client side decorations, am I right?
	},
	// move
	...
	+[](struct wl_client* client, struct wl_resource* resource, int32_t width, int32_t height) {
		...
		// screw you client. I'll make you whatever size I damn well please and there is not a god damn thing you can do
		// about it
	},
	// set_min_size
	+[](struct wl_client* client, struct wl_resource* resource, int32_t width, int32_t height) {
...
XdgShellV6Surface::XdgShellV6Surface(wl_client* client, uint32_t id, uint version, WlSurface surface)
{
	...
	// sending 1 as the version number isn't a mistake. Idk why its called v6 but you send in 1, maybe always 1 until
	// stable?
