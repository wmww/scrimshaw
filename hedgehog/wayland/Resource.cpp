...
struct Resource::Impl
{
	...
	static void destroyCallback(wl_resource* resourceRaw)
	{
		...
		impl->resource =
			nullptr; // this shouldn't be needed as the entire object should be deleted next line, but best to be safe
		impl->data = nullptr;
		Impl::map.erase(iter);
		debug("'" + string(wl_resource_get_class(resourceRaw)) + "' removed, there are now " +
			  to_string(Impl::map.size()) + " resources");
...
void Resource::setup(shared_ptr<Data> dataIn, wl_client* client, uint32_t id, const wl_interface* interface,
					 int version, const void* implStruct)
{
	...
	debug("'" + string(wl_resource_get_class(resource)) + "' created, there are now " + to_string(Impl::map.size()) +
		  " resources");
